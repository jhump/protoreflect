...
//
// For hopefully obvious reasons, enum fields are different than in generated
// code. Named types that are int32s are not accepted as enum values; only
// actual int32 values are accepted.
//
// For other integral types, the types are strict: an int or uint value is not
// allowed when setting a field; it must be an int32, int64, uint32, or uint64.
//
...
//
// 1. A dynamic message is created with a descriptor, A, and then de-serialized
//    from a stream of bytes. The stream includes an unrecognized tag T. The
//    message will include tag T in its unrecognized field set.
// 2. Another call site retrieves a newer descriptor, A', which includes a newly
//    added field with tag T.
// 3. That other call site then uses a FieldDescriptor to access the value of
//    the new field. This will cause the dynamic message to parse the bytes for
//    the unknown tag T and store them as a known field.
// 4. Subsequent operations for tag T, including setting the field using only
//    tag number or de-serializing a stream that includes tag T, will operate as
//    if that tag were part of the original descriptor, A.
//
//
// Compatibility
//
